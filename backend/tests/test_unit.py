...
def test_integrity_signature_matches_spec():
    ref, amount, currency = "ref123", 5000000, "COP"
    expected = hashlib.sha256(
        f"{ref}{amount}{currency}{config.WOMPI_INTEGRITY_SECRET}".encode()
    ).hexdigest()
    assert wompi.integrity_signature(ref, amount, currency) == expected
...
def test_event_signature_verification():
    ...
    concat = "tx1APPROVED5000000" + str(timestamp) + config.WOMPI_EVENTS_SECRET
    ...
    assert wompi.verify_event_signature(payload) is True

    payload["signature"]["checksum"] = "deadbeef"
    assert wompi.verify_event_signature(payload) is False
